...
const ACHIEVEMENT_META: Record<string, {
    ...
    'rank1-march-2026': {
        name: 'Rank 1 March 2026',
        desc: 'Achieve Rank 1 on the leaderboard during March 2026',
        imageSrc: '/images/achievements/rank1_march.webp',
        tier: 'legendary'
    }
};
...
export function AchievementRevealModal({ achievement, onClose, onClaim }: AchievementRevealModalProps) {
    ...
    // If we don't have metadata for this achievement, skip it once (prevents spam when onClose changes)
            ...
            <div className={`relative w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-[2rem] overflow-hidden shadow-2xl ${isAnimating ? 'animate-scale-in' : ''}`}>
                <div className="p-6 text-center">
                    ...
                    <div className="space-y-4">
                        ...
                        <button
                            onClick={handleDismiss}
                            className="w-full py-3.5 bg-[#E8C15A] hover:bg-[#D4AF37] text-black font-black rounded-xl transition-all active:scale-95 hover:shadow-[0_0_20px_rgba(232,193,90,0.3)]"
                        >
                            Claim Achievement
                        </button>
                    </div>
